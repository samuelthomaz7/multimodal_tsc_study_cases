...
class LITELate(NNModel):

    def __init__(self, dataset_name, train_dataset, test_dataset, metadata, model_name = 'LITELate', random_state=42, device='cuda', is_multimodal=True, is_ensemble=True, model_num=None) -> None:
        ...
        for i, input_shape in enumerate(self.input_shape):
            self.modalities[f"modality_{i}"] = LITEMultiplex(
        ...
        #         in_channels=113*self.num_modalities if i == 0 else n_filters, 
        #         out_channels=n_filters,
        #         kernel_size=separable_kernel_size // (2 ** i), 
        ...
        self.linear = nn.Linear(in_features=32*self.num_modalities, out_features= 1 if self.classes_shape[1] == 2 else self.classes_shape[1])
    ...
    def forward(self, x):
        ...
        feature_maps = torch.cat(modality_outputs, dim = 1)
        ...
        out = self.linear(feature_maps)

        return out

class LITEMultiplex(NNModel):

    def __init__(self, modality_shape, dataset_name, train_dataset, test_dataset, metadata, model_name = 'LITE', random_state=42, device='cuda', is_multimodal=False, is_ensemble=True, model_num=None) -> None:
        super().__init__(dataset_name, train_dataset, test_dataset, metadata, model_name, random_state, device, is_multimodal, is_ensemble, model_num)

        self.modality_shape = modality_shape
        custom_kernels_sizes = [2, 4, 8, 16, 32, 64]
        custom_convolutions = []
        
        for ks in custom_kernels_sizes:
            filter_ = np.ones(shape=(1, self.modality_shape[1], ks))
            indices_ = np.arange(ks)

            filter_[:, :, indices_ % 2 == 0] *= -1 # increasing detection filter

            custom_conv = nn.Conv1d(in_channels=self.modality_shape[1], out_channels=1,
        ...
        for ks in custom_kernels_sizes[1:]:
            filter_ = np.zeros(shape=(1, self.modality_shape[1], ks + ks // 2))
        ...
        for i in range(len(inception_kernel_sizes)):
            inception_convolutions.append(
                nn.Conv1d(
                    in_channels=self.modality_shape[1], out_channels=n_filters, kernel_size=inception_kernel_sizes[i],
